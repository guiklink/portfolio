...
My starting objective is to stabilize my AR Parrot using an outside reference (I'm going to start with tags). The starting idea is to use 3 PID controllers, to perform the following functions:
...
## Distance Control
A PID control that keeps the drone in a constant distance from the referential.
