...
What if instead of animating everything, we could use simplified a movement GAIT, simple enough that would not be to hard to implement neither require heavy processing power (therefore not compromising other aspects of the game), but still able to adapt to small peculiarities and somehow convincing. That is the goal of Twig, created by Professor Ian Douglas Horswill from Northwestern University. 
